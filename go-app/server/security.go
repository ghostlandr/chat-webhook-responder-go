...
func unsafeSignatureVerifier(header http.Header, secret, body string) (SecretsVerifier, error) {
	var (
		bsignature []byte
	)
	...
	basestring := fmt.Sprintf("v0:%s:%s", stimestamp, body)
...
func NewSecretsVerifier(header http.Header, secret, body string) (sv SecretsVerifier, err error) {
	var (
		timestamp int64
	)
	...
	if sv, err = unsafeSignatureVerifier(header, secret, body); err != nil {
		return SecretsVerifier{}, err
	}
	...
	if diff > 5*time.Minute {
		return SecretsVerifier{}, errors.New("expired timestamp")
	}
